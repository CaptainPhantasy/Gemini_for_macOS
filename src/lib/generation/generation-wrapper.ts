...
export async function generateWithFailoverStream(
  ...
  const { ai, maxRetries = DEFAULT_MAX_RETRIES, fallbackModel, onChunk, signal } = options;
  const primaryModel = options.model;

  const assertNotAborted = () => {
    if (signal?.aborted) {
      throw new DOMException('Aborted: generateWithFailoverStream cancelled by signal', 'AbortError');
    }
  };
  ...
  const runStream = async (model: string): Promise<GenerationResult & GenerationStreamState> => {
    ...
    const stream = await ai.models.generateContentStream({
      ...
      // SDK accepts an AbortSignal as a top-level field. Per the SDK JSDoc,
      // the abort is client-side only — it stops us from receiving more
      // chunks but does not cancel the upstream network request.
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      // Abort must short-circuit retries: do not retry, do not fall back.
      if (err instanceof DOMException && err.name === 'AbortError') {
        logger.warn(`[failover] Abort signal received; cancelling retries for ${primaryModel}`);
        throw err;
      }
...
export interface GenerationOptions {
  ...
  maxRetries?: number;
  /**
   * Optional AbortSignal used to cancel the in-flight generation.
   * The signal is propagated to the SDK as `abortSignal` (client-side cancel)
   * AND checked between chunks and between retry attempts so the consumer
   * loop can exit synchronously even if the SDK ignores the signal.
   * See docs/plans/chat-barge-and-queue.md §1.3 for the abort-surface decision.
   */
  signal?: AbortSignal;
}
